...
 }
 throw new Error(`Unsupported file type: ${mimeGuess || 'unknown'} (${path.extname(origLower)})`);
}
...
// Key Concept Extraction / Glossary Builder (Feature 1)
...
  const system = `
You are a technical editor. Your task is to identify and define the 10 most important technical terms or concepts from the provided document.
The definitions must be sourced ONLY from the text provided.
...
   "definition": "The definition of the concept, using only the provided text."
  }
 ]
}
`.trim();
...
  res.status(500).json({ error: "Server error generating glossary" });
 }
});

// Interactive Study Path / Learning Plan Generator (Feature 3)
...
  const system = `
You are a study guide generator. Analyze the provided document and create a structured learning plan with 5-7 distinct steps to master the topic.
Each step should build on the last, mention necessary prerequisites or related topics, and suggest an estimated time for completion.

Return ONLY JSON in this exact format:
{
 "planTitle": "Suggested Learning Path: [Main Topic]",
 "steps": [
  {
   "stepNumber": 1,
   "title": "Prerequisite Knowledge Check",
   "description": "Review foundational concepts required to understand the document.",
...
  // Basic validation
  if (!data.planTitle || !data.steps || !Array.isArray(data.steps)) {
   return res.status(500).json({ error: "Invalid response structure from model" });
  }

  res.json({ planTitle: data.planTitle, steps: data.steps });

 } catch (err) {
  console.error("Learning Plan generation error:", err);
  res.status(500).json({ error: "Server error generating learning plan" });
 }
});
